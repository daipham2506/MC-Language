    ...
    *   generate ior
    ...
    def emitREOP(self, op, in_, frame):
        ...
        if op == ">":
            ...
            result.append(self.jvm.emitIFICMPEQ(labelF) if type(in_) in [IntType,BoolType] else compareF + self.jvm.emitIFEQ(labelF)) # =
        elif op == "==":
            result.append(self.jvm.emitIFICMPNE(labelF) if type(in_) in [IntType,BoolType] else compareF + self.jvm.emitIFNE(labelF)) # <>

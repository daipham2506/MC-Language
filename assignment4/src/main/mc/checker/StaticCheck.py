...
class StaticChecker(BaseVisitor,Utils):
    ...
    def visitProgram(self,ast,c):
        list_global = c[:]
        self.list_name_func_check_unreach = [] # reset = null when run new test.
...
#--------------------------------------------------------------------------------------# 
    ...
    def visitArrayCell(self,ast,c):
        if type(ast.arr) is Id :
            res = self.lookup(ast.arr.name, c[0][::-1], lambda x: x.name);
            if res is not None:
                if type(res.mtype) not in [ArrayType, ArrayPointerType] :
                    raise TypeMismatchInExpression(ast) 
        at_arr = self.visit(ast.arr, c)
        at_idx = self.visit(ast.idx, c)
        if isinstance(at_arr,(ArrayType, ArrayPointerType))  and isinstance(at_idx,IntType):
